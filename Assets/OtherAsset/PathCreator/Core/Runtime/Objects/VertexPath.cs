...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        public Vector3 GetRDirectionAtDistance(float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
        {
            float t = 1f - (dst / length);
            return GetRDirection(t, endOfPathInstruction);
        }
        ...
        public Quaternion GetRRotationAtDistance(float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
        {
            float t = 1f - (dst / length);
            return GetRotation(t, endOfPathInstruction);
        }
        ...
        public Vector3 GetRDirection(float t, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
        {
            var data = CalculatePercentOnPathData(t, endOfPathInstruction);
            Vector3 dir = -Vector3.Lerp(localTangents[data.previousIndex], localTangents[data.nextIndex], data.percentBetweenIndices);
            return MathUtility.TransformDirection(dir, transform, space);
